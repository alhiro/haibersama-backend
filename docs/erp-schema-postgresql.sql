...
CREATE TABLE IF NOT EXISTS public.erp_attendance (
  ...
  name VARCHAR(150) NOT NULL,
  description VARCHAR(1000),
  status VARCHAR(40) NOT NULL DEFAULT 'Hadir',
  meta VARCHAR(200),
  amount VARCHAR(100),
  employee VARCHAR(150),
...
CREATE TABLE IF NOT EXISTS public.erp_leave_request (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
CREATE TABLE IF NOT EXISTS public.erp_overtime_request (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
CREATE TABLE IF NOT EXISTS public.erp_employee_task (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
CREATE TABLE IF NOT EXISTS public.erp_work_schedule (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
CREATE TABLE IF NOT EXISTS public.erp_announcement (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
CREATE TABLE IF NOT EXISTS public.erp_payslip (LIKE public.erp_attendance INCLUDING DEFAULTS EXCLUDING INDEXES);
...
BEGIN
  ...
  LOOP
    IF EXISTS (
      ...
      WHERE d.adrelid = to_regclass(format('public.%I', v_table))
        AND a.attname = 'id'
    ) THEN
      EXECUTE format('ALTER TABLE public.%I ALTER COLUMN id DROP DEFAULT', v_table);
    END IF;
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM pg_constraint
      WHERE conrelid = to_regclass(format('public.%I', v_table))
        AND contype = 'p'
    ) THEN
      EXECUTE format('ALTER TABLE public.%I ADD PRIMARY KEY (id)', v_table);
    END IF;
  END LOOP;
END $$;

CREATE INDEX IF NOT EXISTS idx_erp_attendance_partner ON public.erp_attendance (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_attendance_status ON public.erp_attendance (status);
CREATE INDEX IF NOT EXISTS idx_erp_attendance_employee_email ON public.erp_attendance (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_attendance_work_date ON public.erp_attendance (work_date);

CREATE INDEX IF NOT EXISTS idx_erp_leave_request_partner ON public.erp_leave_request (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_leave_request_status ON public.erp_leave_request (status);
CREATE INDEX IF NOT EXISTS idx_erp_leave_request_employee_email ON public.erp_leave_request (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_leave_request_approval ON public.erp_leave_request (approval_status);

CREATE INDEX IF NOT EXISTS idx_erp_overtime_request_partner ON public.erp_overtime_request (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_overtime_request_status ON public.erp_overtime_request (status);
CREATE INDEX IF NOT EXISTS idx_erp_overtime_request_employee_email ON public.erp_overtime_request (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_overtime_request_approval ON public.erp_overtime_request (approval_status);

CREATE INDEX IF NOT EXISTS idx_erp_employee_task_partner ON public.erp_employee_task (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_employee_task_status ON public.erp_employee_task (status);
CREATE INDEX IF NOT EXISTS idx_erp_employee_task_employee_email ON public.erp_employee_task (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_employee_task_due_date ON public.erp_employee_task (due_date);

CREATE INDEX IF NOT EXISTS idx_erp_work_schedule_partner ON public.erp_work_schedule (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_work_schedule_status ON public.erp_work_schedule (status);
CREATE INDEX IF NOT EXISTS idx_erp_work_schedule_employee_email ON public.erp_work_schedule (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_work_schedule_schedule_date ON public.erp_work_schedule (schedule_date);

CREATE INDEX IF NOT EXISTS idx_erp_announcement_partner ON public.erp_announcement (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_announcement_status ON public.erp_announcement (status);
CREATE INDEX IF NOT EXISTS idx_erp_announcement_department ON public.erp_announcement (department);

CREATE INDEX IF NOT EXISTS idx_erp_payslip_partner ON public.erp_payslip (partner_id);
CREATE INDEX IF NOT EXISTS idx_erp_payslip_status ON public.erp_payslip (status);
CREATE INDEX IF NOT EXISTS idx_erp_payslip_employee_email ON public.erp_payslip (employee_email);
CREATE INDEX IF NOT EXISTS idx_erp_payslip_approval ON public.erp_payslip (approval_status);
...
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS latitude VARCHAR(60)', v_table);
    EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS longitude VARCHAR(60)', v_table);
    EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS map_url VARCHAR(500)', v_table);
  END LOOP;
END $$;
