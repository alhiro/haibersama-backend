    ...
    try {
      ...
      return await EventComment.findAndCountAll({
        ...
        repliesCounts.forEach((r) => {
          replyCountMap[r.parent_comment_id] = parseInt(
            r.dataValues.reply_count
          );
        });
        ...
        const responseMessage =
          parent_comment_id === undefined || parent_comment_id === "null"
            ? resp.rows.length > 0
              ? "Semua komentar selayang berhasil diambil!"
              : "Data komen selayang kosong!"
            : resp.rows.length > 0
            ? "Semua balasan komentar selayang berhasil diambil!"
            : "Data komen balasan selayang kosong!";
      ...
      try {
        ...
        if (partnerToken) {
          await admin.messaging().send({
            ...
            android: { notification: { icon: "ic_notification", color: "#1B84FF" } },
            data: { type: "replyComment" },
            token: partnerToken,
          });
          console.log("✅ Notifikasi ke partner sent");
        } else {
          console.log("No token for send messaging");
        }
      } catch (err) {
        console.error("❌ Error FCM partner (not blocking):", err);
      }
